...
sys.path.append('path/to/mPLUG-Owl/mPLUG-Owl')                             # mplug-owl code path
from mplug_owl.modeling_mplug_owl import MplugOwlForConditionalGeneration
from mplug_owl.processing_mplug_owl import MplugOwlImageProcessor, MplugOwlProcessor



pretrained_ckpt = "path/to/MAGAer13/mplug-owl-llama-7b/"           # mplug-owl checkpoint
mllm_model = MplugOwlForConditionalGeneration.from_pretrained(
    pretrained_ckpt,
    torch_dtype=torch.bfloat16,
).to("cuda:0")
image_processor = MplugOwlImageProcessor.from_pretrained(pretrained_ckpt)
mllm_tokenizer = AutoTokenizer.from_pretrained(pretrained_ckpt)
processor = MplugOwlProcessor(image_processor, mllm_tokenizer)
print("Load mPLUG-Owl Done.")
...
def get_mllm_answers(sample, question_key="generated_general_questions", answer_key="answers", additional_sample_counts=0, history=None):
    ...
    for dialogue in sample['dialogues']:
        ...
        for cur_qs in gen_qs:
            qs, entity = cur_qs
            answer = get_owl_output(img_path, qs, history=history)
            cur_answers.append(answer.strip())
            for _ in range(additional_sample_counts):                       # get sample answers
                answer = get_owl_output(img_path, qs, do_sample=True, history=history)
                cur_answers.append(answer.strip())
...
def get_owl_output(img_path, question, do_sample=False, history=None):
    if history is None:
        prompts = [PROMPT_TEMPLATE.format(question=question)]
    else:
        prompts = [PROMPT_HISTORY_TEMPLATE.format(history=history, question=question)]
        print(prompts)
    prompts = [PROMPT_TEMPLATE.format(question=question)]
    image_list = [img_path]
    ...
    inputs = {k: v.to(mllm_model.device) for k, v in inputs.items()}

    with torch.no_grad():
        res = mllm_model.generate(**inputs, **generate_kwargs)
    sentence = mllm_tokenizer.decode(res.tolist()[0], skip_special_tokens=True)
    return sentence
...
def main(type):
    ...
    output_file = "./result/logs_mplug_pope/answer_%s_logic_check.json" % (type)           # output file
    torture_log = "./result/logs_mplug_pope/torture_%s_%d.json"                        # log file
    ...
    for idx, question in enumerate(questions):
        ...
        claim = get_owl_output(temp_image_path, text)
        sample = {  
            'query': text,
