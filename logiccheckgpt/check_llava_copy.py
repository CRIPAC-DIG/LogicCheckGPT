...
def get_mllm_answers(sample, question_key="generated_general_questions", answer_key="answers", additional_sample_counts=0):
    """
...
def get_llava_output(img_path, question, do_sample=False):
    ...
    if isinstance(img_path, str) and os.path.isfile(img_path):
        ...
        image = img_path
    image_tensor = image_processor.preprocess(image, return_tensors='pt')['pixel_values'][0]                   # ['pixel_values'][0]
...
def main(type):
    ...
    questions = [json.loads(q) for q in open(question_file, 'r')][:10]
    answers = []
